...
<div class="container">
    ...
    <div class="row border border-primary">
        <div class="col">
            <p>
                {{ __($arriendo->inquilino->primerNombre.' '.$arriendo->inquilino->segundoNombre.' '.$arriendo->inquilino->primerApellido.' '.$arriendo->inquilino->segundoApellido) }}
            </p>
            <p>{{ __( $arriendo->inquilino->telefono) }}</p>
            <p>{{ __( $arriendo->inquilino->email) }}</p>
        </div>
        <div class="col">
            <!--Mejorar esta parte-->
            <p>{{ __('Características del arriendo: '.$arriendo->inmueble->caracteristicas) }}</p>
        </div>
        <div class="col">
            <ul>
                ...
                @php
                    ...
                    $solicitudesRechazadas = $arriendo->solicitudesFinalizacion->where('rutEmisor', Auth::user()->rut)->where('respuesta', 'false');
                @endphp
                ...
                @endif
                @if (count($solicitudesRechazadas))
        ...
        <tbody>
            ...
            <tr @if ($deuda->estado) class="table-success" @else class="table-active" @endif>
                ...
                <td>
                    ...
                    <a href="{{ '/deuda/pago/problema/reportar/'.$deuda->id }}" class="btn btn-primary">Repotar Problema</a>
                    @endif
                </td>
            </tr>
            @endforeach
...
<script type="text/javascript">
    function solicitarFinalizacion(arriendo) {
        ...
        /*var form = $('<form action="/solicitud" mathod="POST"><form>');
        var token = $('<input type="hidden" name="_token" value="{{ csrf_token() }}">');*/
        //var id = $('<input type="hidden" id="id" value="' + arriendo.id + '">');
        ...
        //form.append(token, id, motivoDiv, fechaDiv);
    ...
    function responderSolicitud(solicitud) {
        ...
        //var id = $('<input type="hidden" id="id" value="' + solicitud.id + '">');
